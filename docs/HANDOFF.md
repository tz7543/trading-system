...
**日期：** 2026-06-11
**狀態：** 稽核 backlog（M2/M3）五項已完成、README 已移除；Paper TWS manual smoke test 待執行
**上一版：** 2026-06-10（實盤路徑硬化 PR #1 合併版）
...
**已完成（2026-06-11，五項；tests 315 → 353）：**

| 項目 | 實作 | 關鍵檔案 |
|---|---|---|
| H4 決策日誌無條件記錄 | `DecisionLogger.log()` 接受 `market=None`（行情欄位記 NULL，決策結果/理由必留）；`RiskPipeline._log_decision` 移除 `and market` 守衛；DuckDB schema 本無 NOT NULL，免遷移 | `storage/decision_logger.py`、`assembly.py` |
| H5/M4 TickWriter 非阻塞 flush | Parquet I/O 移到 `asyncio.to_thread`；per-partition `asyncio.Lock` 序列化；`flush()`/`close()` 改 **async** 並先等待 in-flight 自動 flush task（durability）；新增記憶體 bytes 門檻 `flush_interval_bytes`（預設 10MB） | `storage/tick_writer.py`、`subscriber.py` |
| M6 歷史資料缺失 fail-fast | 合約目錄完全不存在 → raise `DataMissingError`（訊息含 symbol/sec_type/預期路徑） | `market_data/exceptions.py`、`historical.py` |
| option_chain / throttler 補測試 | 鏈查詢轉換、qualify/con_id 填入、空結果路徑；pacing 間隔（patch sleep）、並發節流 | `tests/test_option_chain.py`、`tests/test_throttler.py` |
| metrics fsum + converters warning | 加總改 `math.fsum`；ticker timestamp 缺失記 `logger.warning`（fallback datetime.now(UTC) 行為不變） | `backtest/metrics.py`、`tws_client/converters.py` |

對抗式驗證殘留的 minor 觀察（接受不修，後續參考）：

- **`RequestThrottler.active_requests` 是 source bug**：回傳「可用許可數」而非
  「使用中請求數」（`throttler.py` semaphore `_value`）。測試刻意釘住現行為；
  修正時必須同步改 `test_active_requests_property_returns_semaphore_available_permits`。
- TickWriter flush 錯誤路徑：buffer 在 I/O 前清空，寫檔失敗（如磁碟滿）該批
  rows 永久遺失、例外被吞（與舊版 fire-and-forget 一致）。
- TickWriter 超過門檻後每次 write 都會再 spawn flush task，event loop 飢餓時
  可能堆積（task 有以 `_pending_tasks` 追蹤，有界於 write 次數）。
- M6 語意邊界：合約目錄存在但無 `date=` 子目錄 → 仍靜默回空（涵蓋正常假日
  缺口；TickWriter 建目錄必帶 date 分區，實務上窄）。
- converters timestamp warning 無去重——訂閱初期 snapshot ticker 常 time=None，
  可能短暫連發。
- H4：market=None 時 symbol 欄記 NULL（可由 signal 推導，留待需要 per-symbol
  決策分析時再補）。
...
- `README.md` 已於 2026-06-11 刪除（稽核裁定），AGENTS.md 為單一資訊源
  （caveat 已同步更新）。
