...
- **AlertEvent value 慣例**：營運類警報（拒單、重連）`value=0.0`；
  數值類警報（回撤、greeks、margin）沿用實際數值；
  watchdog 帶 `value=age`（秒），具操作價值（PR review 迭代採納）。
...
**PR review（opus + codex）**：採納——late-fill 保留 fillEvent handler（`_cleanup_order`
僅拆 statusEvent，fillEvent 因 TWS 無序保留以接收遲到 execDetails）、margin_info 進
pre-trade（`AccountState.margin_info()` 回傳 `MarginInfo`；`RiskPipeline` 新增
`margin_info_provider`，`on_signal` 透傳至 `PreTradeValidator.validate()`；live 接線
`account_state.margin_info`，backtest 維持 `None`）、watchdog per-contract key（以
`contract_key(event.contract)` 代替裸 `event.symbol`，AlertEvent message 含 key）、
AlertEvent value 慣例修訂（watchdog 帶 `value=age` 秒，見 §3.2）；平倉訂單計數為
已知限制不重審。
